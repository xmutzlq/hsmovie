...
class MovieDetailControllerLogic extends GetxController
    with StateMixin<MovieDetailEntity> {
  ...
  final GlobalKey<FixVerticalCardPagerState> slideKey =
      GlobalKey<FixVerticalCardPagerState>();
  ...
  void onReady() {
    ...
    getMovieDetailRemoteData(_movieId!);
    super.onReady();
  }
  ...
  void getMovieDetailRemoteData(String movieId) async {
    ...
    if (!kIsWeb) {
      ...
      if (movieDetailData.ok) {
        MovieDetailEntity entity = MovieDetailEntity.fromJson(
  ...
  void retryMovieDetail() {
    final movieId = _movieId;
    if (movieId != null) getMovieDetailRemoteData(movieId);
  }
  ...
  Future<void> updateFilmBoxBadge([int? filmSize]) async {
    ...
    if (!cartState.mounted || !identical(cartKey.currentState, cartState))
      return;
  ...
  Future<List<MovieInfo>> getFilmsInBox() async {
    ...
    FilmBoxInfo filmBoxInfo =
        filmBox.get(BOX_KEY_FILM_BOX) ?? FilmBoxInfo(filmBoxRecord: []);
  ...
  void saveBrowsingRecord() {
    MovieDetailEntity? entity = detailState.entity;
    if (entity != null) {
      personalLogic.saveBrowsingRecord(
